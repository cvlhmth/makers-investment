...
const PAYMENT_STATUS_OPTIONS = ["Pendente", "Pago"];
...
function bindEvents() {
  ...
  els.statusCatmanFilter.addEventListener("change", () => {
    state.filters.statusCatman = els.statusCatmanFilter.value;
    applyFiltersAndRender();
  });
  ...
  els.clearFiltersButton.addEventListener("click", () => {
    state.filters = { search: "", maker: "", month: "", year: "", catman: "", status: "", statusCatman: "", statusNd: "", payment: "", quick: "" };
...
function applyFiltersAndRender() {
  ...
  const statusCatmanKey = findStatusCatmanKey();
  const statusNdKey = findStatusNdKey();
  const paymentKey = findPaymentValueKey();
  ...
  let rows = state.rows.filter((row) => {
    ...
    if (state.filters.payment && getPaymentStatusBucket(row[paymentKey]) !== state.filters.payment) return false;
...
function syncFilterOptions() {
  ...
  fillSelect(els.paymentFilter, "Todos", PAYMENT_STATUS_OPTIONS, toDisplayCase);
}
...
async function sendSheetUpdate(row, column, value) {
  ...
  if (!maker) {
    setWriteStatus("Edi��o local, Nome n�o encontrado", "error");
    return;
  }
...
function getPaymentStatusBucket(value) {
...
function findStatusCatmanKey() {
  const exact = state.columns.find((column) => normalizeHeader(column.label) === "status catman");
  if (exact) return exact.key;

  const partial = state.columns.find((column) => {
    const normalized = normalizeHeader(column.label);
    return normalized.includes("status") && normalized.includes("catman");
  });
